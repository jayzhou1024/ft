...
/*
version5
在version4的基础上 将part_op改为用移位来计算出
mov_to_vlr的参数
*/
...
float norm2(vector float *v, vector float *uv, int vBlocks, int shift) {
  int i, loop, b1, b2;
  float res;
...
void update_qr(vector float *v1, vector float *v2, int vBlocks, int shift,
...
int DSPF_sp_qrd(const int Nrows, const int Ncols, vector float *r,
  ...
  int row, col, loop_count, shift, vBlocks;
  float alpha, scale, scale_tmp, sum, diag;
  int Nvecs ; // 处理的向量数
  //int Ri ;                   // Vec matrix_index
  //printf("\n op_sum:  ");
  if (Nrows <= Ncols) {
    loop_count = Nrows - 2;
  } else {
  ...
  for (col = 0; col <= loop_count; col++) {
    sum = 0;
    Nvecs = (Nrows + 15-col) / 16;
    shift = (16-(Nrows - col )% 16)%16;
   ...
   //printf("round:%d> %f\t ", col, sum);
    if (sum != 0) {
      ...
      if (scale_tmp != 0.0) {
        ...
        update_qr(tmpp, uv, Nvecs , shift, Ncols - col - 1, Nrows, scale);
        tmpp = (vector float *)((float *)q + col);
        update_qr(tmpp, uv, Nvecs , shift, Nrows, Nrows, scale);
      } /* if (norm_sqr!=0) */
      //printf("\n");
    }   /* if (sum!=0) */
