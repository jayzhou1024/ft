...
void update_r(vector float *v1, vector float *sum, float *u, int vBlocks,
  ...
  for (i = 0; i < rows; i++) {
    ...
    for (j = 0; j < in_loop; j++) {
      ...
      sum[b1] = vec_mula(v1[b1], cast, sum[b1]);
      sum[b2] = vec_mula(v1[b2], cast, sum[b2]);
      sum[b3] = vec_mula(v1[b3], cast, sum[b3]);
    }
    for (; j < vBlocks - 1; j++) {
  ...
  for (j = 0; j < in_loop; j++) {
    ...
    sum[b1] = vec_muli(v_scale, sum[b1]);
    sum[b2] = vec_muli(v_scale, sum[b2]);
    sum[b3] = vec_muli(v_scale, sum[b3]);
  }
  for (; j < vBlocks - 1; j++) {
  ...
  for (i = 0; i < rows; i++) {
    ...
    for (j = 0; j < in_loop; j++) {
      ...
      v1[b1] = vec_mulb(cast, sum[b1], v1[b1]);
      v1[b2] = vec_mulb(cast, sum[b2], v1[b2]);
      v1[b3] = vec_mulb(cast, sum[b3], v1[b3]);
...
void update_q(vector float *v1, vector float *v2, int vBlocks, int shift,
  ...
  int i, j, in_loop, b1, b2, b3;
  volatile vector float sum1_1, sum1_2, sum1_3, vzero;
  ...
  for (i = 0; i < rows; i++) {
    ...
    for (; j < in_loop; j++) {
      ...
      sum1_1 = vec_mula(v1[b1], v2[b1], sum1_1);
      sum1_2 = vec_mula(v1[b2], v2[b2], sum1_2);
      sum1_3 = vec_mula(v1[b3], v2[b3], sum1_3);
    }
    for (; j < vBlocks - 1; j++) {
    ...
    for (; j < in_loop; j++) {
      ...
      v1[b1] = vec_mulb(v2[b1], sum1_1, v1[b1]);
      v1[b2] = vec_mulb(v2[b2], sum1_1, v1[b2]);
      v1[b3] = vec_mulb(v2[b3], sum1_1, v1[b3]);
