...
void DSP_fir_r8_h24_cn ( const short * x,const short * h,short* r,   int nr);
void DSP_fir_r8_h24_asm(
	...
	*cache1=0x1;
	...
	for(nr = 8; nr <= NR; nr += 4) {
            ...
            */
            ...
            printf("   \tNR = %d\t  cn_time:%d\t  vc_cycle:%d\t\n", nr,time_cn,time_vc);
            vfree(xAddr);
            vfree(rAddr);
	}
}
