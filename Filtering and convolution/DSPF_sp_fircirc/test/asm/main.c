...
void main(){
	...
	*cache1=0x1;
    ...
    /* 测试 */
   for(index_i = 2; index_i < INDEX; index_i++){
        for(nc = 5; nc < NC; nc++){
            for(nr = 16; nr <= NR; nr += 16) {   
                for(nh = 16; nh <= NH; nh += 2) {  
                    //   nr = 32; 
                    //   nh = 4;
                    ...
                    if (max_pct_diff > 0.01)
                        ...
                        printf("\t index = %-3d\t NC = %-3d \tNX = %-3d\t  NH = %-2d\t cn_cycle:%-4d\t  vc_cycle:%-4d\t \n", index_i,nc,nr, nh,time_cn-c_time,time_vc-c_time);
                }
            }
         }
     }
...
unsigned long  getTime_vc(vector float * x,float *h,vector float* r,int nh,int nr,int index,int csize){
	...
	DSPF_sp_fircirc_asm(x,h,r,index,csize,nh,nr);
