...
;         VR7~12     temp1:h[i]??????  temp2 temp3 temp4??????x?????????? temp5 temp6  temp7??????r??????????
;
;		  R40: R21<<2  ???????????????·??????????????   i<<2
;         R42: R20<<2  ???????????????·??????????????   j<<2
;		  R44:  ??????????????x?????????????????· R45:R44=>R11:R10+(i+j)*4    
; 		  R46: k<<4    ??????????????r??????????????   
;
;??????????????????????????????????????????
	    SMOVIL		15, R42
...
|		SMOVIH			0, R47
|		VMOVIH			0, VR11
		SMVCGC.L		R42, VLR      ;P5 SIEU
|		SSUB.M1			1,R16,R16
		

;part3:
;??????????????
dspf_sp_fir_gen_LOOP1:    ;11
		SLDW			*AR10++[1],R36     ;p6
...
|		SMOVIH			0, R32
dspf_sp_fir_gen_LOOP2:   ;
		SSUB.M1 		R32,R18,R38  ;nr-j
		SSHFLL			2, R32,R42   ;j*4
		SADD.M2			R42,R44,R48  ;x+i+j	
|		SSHFLL			4, R34,R46	 ;4<<k  
		SADD.M2		    R14,R46,R46	 
|       SMVAGA36.M1 	R49:R48, AR3 ;  x+i+j???????·   ??????????????????????????16  ????????OR3
		SMVAGA36.M1 	R47:R46, AR4 ;r+k?????????·
		SMVAGA36.M1 	R47:R46, AR5		
;       VR7~12
;		AR3 ??????????????x???????????  AR4??????????????r???????????   OR3??????OR3:  
;		h VR3   
...
|		SADD.M2			12,R32,R32           ;j+=12
		SLT				R32,R18,R0   ;???????? nr j   SIEU
		SNOP  			4			
		VFMULAS32.M1	VR7, VR3, VR8, VR8  
