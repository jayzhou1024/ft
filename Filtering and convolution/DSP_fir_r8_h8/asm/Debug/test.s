	.file	"test.c"
.text;
.Ltext0:
	.section	.text.DSP_fir_r8_h8_vc1,"ax",@progbits
	.align	2
	.global	DSP_fir_r8_h8_vc1
	.type	DSP_fir_r8_h8_vc1, @function
DSP_fir_r8_h8_vc1:
.LFB3:
	.file 1 "../test.c"
	.loc 1 17 0
		SMOVIL		768, R6
		SMOVIL		0, R7
		SSUBA.L		R7:R6, AR7, AR7
		SNOP		2
		SMVAAGL.M1		AR14, R7:R6
		SNOP		1
		SMVAAGH.M1		AR14, R7:R6
		SNOP		1
		SSTDW		R7:R6, *AR15--[2]
.LCFI0:
		SADDA.M1		16,AR15,AR14
.LCFI1:
		SNOP		1
		SMOVIL		-224, R6
		SMOVIL		-1, R7
		SADDA.M1		R7:R6,AR15,AR15
		SNOP		1
		SSTDW		R63:R62, *+AR15[28]
		SSTDW		R39:R38, *+AR15[27]
		SSTDW		R37:R36, *+AR15[26]
		SSTDW		R35:R34, *+AR15[25]
		SSTDW		R33:R32, *+AR15[24]
		SSTDW		R31:R30, *+AR15[23]
.LCFI2:
		SMOVIL		-152, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R38
		SMOVIL		-1, R39
		SADDA.M1		R39:R38,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTDW		R11:R10, *AR10
		SMOVIL		-160, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTDW		R13:R12, *AR10
		SMOVIL		-168, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTDW		R15:R14, *AR10
		SMOVIL		-172, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R16, *AR10
	.loc 1 19 0
		SMOVIL		1075183616, R42
		SMOVIH		1075183616, R42
		SMOVIL		0, R43
		SSTDW		R43:R42, *-AR14[10]
	.loc 1 21 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMOVIL		67305985, R10
		SMOVIH		67305985, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 22 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		4,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		134678021, R10
		SMOVIH		134678021, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 23 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		8,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		202050057, R10
		SMOVIH		202050057, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 24 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		12,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		269422093, R10
		SMOVIH		269422093, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 26 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		16,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		100992003, R10
		SMOVIH		100992003, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 27 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		20,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		168364039, R10
		SMOVIH		168364039, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 28 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		24,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		235736075, R10
		SMOVIH		235736075, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 29 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		28,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		303108111, R10
		SMOVIH		303108111, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 34 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,32,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		134678021, R10
		SMOVIH		134678021, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 35 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,36,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		202050057, R10
		SMOVIH		202050057, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 36 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,40,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		269422093, R10
		SMOVIH		269422093, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 37 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,44,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		336794129, R10
		SMOVIH		336794129, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 39 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,48,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		168364039, R10
		SMOVIH		168364039, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 40 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,52,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		235736075, R10
		SMOVIH		235736075, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 41 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,56,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		303108111, R10
		SMOVIH		303108111, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 42 0
		SLDDW		*-AR14[10], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,60,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		370480147, R10
		SMOVIH		370480147, R10
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTW		R10, *AR10
	.loc 1 47 0
		SMOVIL		-172, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SADD.M1		15,R42,R42
		SSHFAR		31, R42, R43
		SSHFLR		28, R43, R43
		SADD.M1		R42,R43,R42
		SSHFAR		4, R42, R42
		SMOVIL		-84, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SSTW		R42, *AR10
	.loc 1 58 0
		SMOVIL		-168, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SSTDW		R43:R42, *-AR14[12]
	.loc 1 71 0
		SMVAAGL.M1		AR7, R11:R10
		SNOP		1
		SMVAAGH.M1		AR7, R11:R10
		SNOP		1
		SSTDW		R11:R10, *-AR14[13]
	.loc 1 74 0
		SLDDW		*-AR14[13], R43:R42
		SNOP		5
		SSTDW		R43:R42, *-AR14[14]
	.loc 1 75 0
		SLDDW		*-AR14[13], R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,64,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SSTDW		R43:R42, *-AR14[15]
	.loc 1 81 0
		SMOVIL		-116, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*AR10, R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 82 0
		SMOVIL		-116, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		2,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*AR10, R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 83 0
		SMOVIL		-116, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SVBCAST.M1 		 R42,VR3
		SNOP		3
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 84 0
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		SMOVIL		736, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTH.LS 		VR3,*AR1
	.loc 1 86 0
		SMOVIL		-120, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[1], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 87 0
		SMOVIL		-120, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		2,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[1], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 88 0
		SMOVIL		-120, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SVBCAST.M1 		 R42,VR3
		SNOP		3
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 89 0
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		SMOVIL		704, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTH.LS 		VR3,*AR1
	.loc 1 91 0
		SMOVIL		-124, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[2], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 92 0
		SMOVIL		-124, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		2,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[2], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 93 0
		SMOVIL		-124, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SVBCAST.M1 		 R42,VR3
		SNOP		3
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 94 0
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		SMOVIL		672, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTH.LS 		VR3,*AR1
	.loc 1 96 0
		SMOVIL		-128, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[3], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 97 0
		SMOVIL		-128, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		2,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[3], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 98 0
		SMOVIL		-128, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SVBCAST.M1 		 R42,VR3
		SNOP		3
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 99 0
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		SMOVIL		640, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTH.LS 		VR3,*AR1
	.loc 1 101 0
		SMOVIL		-132, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[4], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 102 0
		SMOVIL		-132, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		2,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[4], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 103 0
		SMOVIL		-132, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SVBCAST.M1 		 R42,VR3
		SNOP		3
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 104 0
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		SMOVIL		608, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTH.LS 		VR3,*AR1
	.loc 1 106 0
		SMOVIL		-136, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[5], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 107 0
		SMOVIL		-136, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		2,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[5], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 108 0
		SMOVIL		-136, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SVBCAST.M1 		 R42,VR3
		SNOP		3
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 109 0
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		SMOVIL		576, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTH.LS 		VR3,*AR1
	.loc 1 111 0
		SMOVIL		-140, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[6], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 112 0
		SMOVIL		-140, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		2,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[6], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 113 0
		SMOVIL		-140, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SVBCAST.M1 		 R42,VR3
		SNOP		3
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 114 0
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		SMOVIL		544, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTH.LS 		VR3,*AR1
	.loc 1 116 0
		SMOVIL		-144, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[7], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 117 0
		SMOVIL		-144, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		2,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R10
		SMOVIL		-1, R11
		SMOVIL		-56, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,OR11
		SNOP		1
		SADDA.M1		R11:R10,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R11:R10
		SNOP		1
		SMVAAGH.M1		OR11, R11:R10
		SNOP		1
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, AR10
		SNOP		1
		SLDH		*+AR10[7], R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SSTH		R10, *AR10
	.loc 1 118 0
		SMOVIL		-144, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SVBCAST.M1 		 R42,VR3
		SNOP		3
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 119 0
		SMOVIL		-128, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		SMOVIL		512, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTH.LS 		VR3,*AR1
	.loc 1 121 0
		SMOVIL		-152, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMOVIL		-136, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SSTDW		R43:R42, *AR10
	.loc 1 122 0
		SMOVIL		-152, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMOVIL		-144, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SSTDW		R43:R42, *AR10
	.loc 1 124 0
		SMOVIL		0, R42
		SMOVIL		-68, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SSTW		R42, *AR10
		SMOVIL		0, R42
		SMOVIL		-64, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SSTW		R42, *AR10
		SBR		.L2
		SNOP		6
.L3:
	.loc 1 127 0
		SMOVIL		-68, R42
		SMOVIL		-1, R43
		SADDA.M1		R43:R42,AR14,AR10
		SNOP		1
		SLDW		*AR10, R24
		SNOP		5
		SMOVIL		-68, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SSHFAR		31, R42, R42
		SMOV.M1		R42, R25
		SSHFLR		26, R24, R42
		SSHFLL		6, R25, R45
		SOR		R45, R42, R45
		SSHFLL		6, R24, R44
		SMOV.M1		R44, R42
		SMOV.M1		R45, R43
	;no-op trunc di R43:R42 to pdi R43:R42
		SLDDW		*-AR14[12], R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDW.LS 		*AR1,VR3
		SNOP		7
		VSTW.LS 		VR3,*+AR7[32]
	.loc 1 128 0
		SMOVIL		-136, R42
		SMOVIL		-1, R43
		SADDA.M1		R43:R42,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMOVIL		-64, R42
		SMOVIL		-1, R43
		SADDA.M1		R43:R42,AR14,AR10
		SNOP		1
		SLDW		*AR10, R28
		SNOP		5
		SMOVIL		-64, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SSHFAR		31, R42, R42
		SMOV.M1		R42, R29
		SMOV.M1		R28, R42
		SMOV.M1		R29, R43
	;no-op trunc di R43:R42 to pdi R43:R42
		SSHFLR		31, R42, R12
		SSHFLL		1, R43, R53
		SOR		R53, R12, R53
		SSHFLL		1, R42, R52
		SMVAGA36.M1		R11:R10, OR11
		SNOP		1
		SADDA.M1		R53:R52,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		VSTH.LS 		VR3,*+AR7[240]
	.loc 1 129 0
		VLDH.LS 		*+AR7[240],VR4
		SNOP		7
		SMOVIL		736, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR5
		SNOP		7
		VLDW.LS 		*+AR7[32],VR3
		SNOP		7
		VMULAS16T.M1		 VR4,VR5,VR3,VR3
		SNOP		2
		VSTW.LS 		VR3,*+AR7[32]
	.loc 1 131 0
		SMOVIL		-64, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SADD.M1		2,R42,R42
		SMOVIL		-136, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMOV.M1		R42, R8
		SSHFAR		31, R42, R42
		SMOV.M1		R42, R9
		SMOV.M1		R8, R42
		SMOV.M1		R9, R43
	;no-op trunc di R43:R42 to pdi R43:R42
		SSHFLR		31, R42, R12
		SSHFLL		1, R43, R55
		SOR		R55, R12, R55
		SSHFLL		1, R42, R54
		SMVAGA36.M1		R11:R10, OR11
		SNOP		1
		SADDA.M1		R55:R54,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		VSTH.LS 		VR3,*+AR7[208]
	.loc 1 132 0
		VLDH.LS 		*+AR7[208],VR4
		SNOP		7
		SMOVIL		672, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR5
		SNOP		7
		VLDW.LS 		*+AR7[32],VR3
		SNOP		7
		VMULAS16T.M1		 VR4,VR5,VR3,VR3
		SNOP		2
		VSTW.LS 		VR3,*+AR7[32]
	.loc 1 134 0
		SMOVIL		-64, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SADD.M1		4,R42,R42
		SMOVIL		-136, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMOV.M1		R42, R30
		SSHFAR		31, R42, R42
		SMOV.M1		R42, R31
		SMOV.M1		R30, R42
		SMOV.M1		R31, R43
	;no-op trunc di R43:R42 to pdi R43:R42
		SSHFLR		31, R42, R12
		SSHFLL		1, R43, R57
		SOR		R57, R12, R57
		SSHFLL		1, R42, R56
		SMVAGA36.M1		R11:R10, OR11
		SNOP		1
		SADDA.M1		R57:R56,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		VSTH.LS 		VR3,*+AR7[176]
	.loc 1 135 0
		VLDH.LS 		*+AR7[176],VR4
		SNOP		7
		SMOVIL		608, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR5
		SNOP		7
		VLDW.LS 		*+AR7[32],VR3
		SNOP		7
		VMULAS16T.M1		 VR4,VR5,VR3,VR3
		SNOP		2
		VSTW.LS 		VR3,*+AR7[32]
	.loc 1 137 0
		SMOVIL		-64, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SADD.M1		6,R42,R42
		SMOVIL		-136, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMOV.M1		R42, R32
		SSHFAR		31, R42, R42
		SMOV.M1		R42, R33
		SMOV.M1		R32, R42
		SMOV.M1		R33, R43
	;no-op trunc di R43:R42 to pdi R43:R42
		SSHFLR		31, R42, R12
		SSHFLL		1, R43, R59
		SOR		R59, R12, R59
		SSHFLL		1, R42, R58
		SMVAGA36.M1		R11:R10, OR11
		SNOP		1
		SADDA.M1		R59:R58,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		VSTH.LS 		VR3,*+AR7[144]
	.loc 1 138 0
		VLDH.LS 		*+AR7[144],VR4
		SNOP		7
		SMOVIL		544, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR5
		SNOP		7
		VLDW.LS 		*+AR7[32],VR3
		SNOP		7
		VMULAS16T.M1		 VR4,VR5,VR3,VR3
		SNOP		2
		VSTW.LS 		VR3,*+AR7[32]
	.loc 1 141 0
		SMOVIL		-64, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SSHFAR		1, R42, R42
		SMOVIL		-144, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMOV.M1		R42, R34
		SSHFAR		31, R42, R42
		SMOV.M1		R42, R35
		SMOV.M1		R34, R42
		SMOV.M1		R35, R43
	;no-op trunc di R43:R42 to pdi R43:R42
		SSHFLR		30, R42, R12
		SSHFLL		2, R43, R61
		SOR		R61, R12, R61
		SSHFLL		2, R42, R60
		SMVAGA36.M1		R11:R10, OR11
		SNOP		1
		SADDA.M1		R61:R60,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDW.LS 		*AR1,VR3
		SNOP		7
		VSTW.LS 		VR3,*+AR7[48]
	.loc 1 144 0
		SMOVIL		0, R42
		SMVCGC.L		R42, SMR
		SNOP		2
	.loc 1 145 0
		SMOVIL		192, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDW.LS 		*AR1,VR3
		SNOP		7
		SMOVIL		192, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDW.LS 		*AR1,VR4
		SNOP		7
		VSHUFH		 VR3,VR4,VR3
		SNOP		2
		VSTW.LS 		VR3,*+AR7[48]
	.loc 1 147 0
		SMOVIL		192, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		VSTH.LS 		VR3,*+AR7[224]
	.loc 1 148 0
		SMOVIL		192, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,32,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		VSTH.LS 		VR3,*+AR7[192]
	.loc 1 150 0
		VLDH.LS 		*+AR7[224],VR4
		SNOP		7
		SMOVIL		704, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR5
		SNOP		7
		VLDW.LS 		*+AR7[32],VR3
		SNOP		7
		VMULAS16T.M1		 VR4,VR5,VR3,VR3
		SNOP		2
		VSTW.LS 		VR3,*+AR7[32]
	.loc 1 151 0
		VLDH.LS 		*+AR7[192],VR4
		SNOP		7
		SMOVIL		640, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR5
		SNOP		7
		VLDW.LS 		*+AR7[32],VR3
		SNOP		7
		VMULAS16T.M1		 VR4,VR5,VR3,VR3
		SNOP		2
		VSTW.LS 		VR3,*+AR7[32]
	.loc 1 154 0
		SMOVIL		2, R42
		SMVCGC.L		R42, SMR
		SNOP		2
	.loc 1 155 0
		SMOVIL		-64, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SSHFAR		1, R42, R42
		SMOVIL		-144, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SMOV.M1		R42, R36
		SSHFAR		31, R42, R42
		SMOV.M1		R42, R37
		SMOV.M1		R36, R42
		SMOV.M1		R37, R43
	;no-op trunc di R43:R42 to pdi R43:R42
		SSHFLR		30, R42, R12
		SSHFLL		2, R43, R19
		SOR		R19, R12, R19
		SSHFLL		2, R42, R18
		SMVAGA36.M1		R11:R10, OR11
		SNOP		1
		SADDA.M1		R19:R18,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDW.LS 		*AR1,VR3
		SNOP		7
		VSTW.LS 		VR3,*+AR7[48]
	.loc 1 156 0
		SMOVIL		192, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDW.LS 		*AR1,VR3
		SNOP		7
		SMOVIL		192, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDW.LS 		*AR1,VR4
		SNOP		7
		VSHUFH		 VR3,VR4,VR3
		SNOP		2
		VSTW.LS 		VR3,*+AR7[48]
	.loc 1 158 0
		SMOVIL		192, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		VSTH.LS 		VR3,*+AR7[160]
	.loc 1 159 0
		SMOVIL		192, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,32,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR3
		SNOP		7
		VSTH.LS 		VR3,*+AR7[128]
	.loc 1 161 0
		VLDH.LS 		*+AR7[160],VR4
		SNOP		7
		SMOVIL		576, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR5
		SNOP		7
		VLDW.LS 		*+AR7[32],VR3
		SNOP		7
		VMULAS16T.M1		 VR4,VR5,VR3,VR3
		SNOP		2
		VSTW.LS 		VR3,*+AR7[32]
	.loc 1 162 0
		VLDH.LS 		*+AR7[128],VR4
		SNOP		7
		SMOVIL		512, R42
		SMOVIL		0, R43
		SADDA.M1		R43:R42,AR7,OR0
		SNOP		2
		SMVAAGL.M1		OR0, R43:R42
		SNOP		1
		SMVAAGH.M1		OR0, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDH.LS 		*AR1,VR5
		SNOP		7
		VLDW.LS 		*+AR7[32],VR3
		SNOP		7
		VMULAS16T.M1		 VR4,VR5,VR3,VR3
		SNOP		2
		VSTW.LS 		VR3,*+AR7[32]
	.loc 1 164 0
		SMOVIL		-68, R42
		SMOVIL		-1, R43
		SADDA.M1		R43:R42,AR14,AR10
		SNOP		1
		SLDW		*AR10, R26
		SNOP		5
		SMOVIL		-68, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SSHFAR		31, R42, R42
		SMOV.M1		R42, R27
		SSHFLR		26, R26, R42
		SSHFLL		6, R27, R51
		SOR		R51, R42, R51
		SSHFLL		6, R26, R50
		SMOV.M1		R50, R42
		SMOV.M1		R51, R43
	;no-op trunc di R43:R42 to pdi R43:R42
		SLDDW		*-AR14[12], R11:R10
		SNOP		5
		SMVAGA36.M1		R11:R10, OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		VLDW.LS 		*+AR7[32],VR3
		SNOP		7
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 124 0
		SMOVIL		-64, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SADD.M1		16,R42,R42
		SMOVIL		-64, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SSTW		R42, *AR10
		SMOVIL		-68, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SADD.M1		1,R42,R42
		SMOVIL		-68, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,AR10
		SNOP		1
		SSTW		R42, *AR10
.L2:
		SMOVIL		-172, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-64, R12
		SMOVIL		-1, R13
		SADDA.M1		R13:R12,AR14,AR10
		SNOP		1
		SLDW		*AR10, R10
		SNOP		5
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SLT		R10, R42, R42
		SMOV.M1		R42, R0
	[R0]	SBR		.L3
		SNOP		6
	.loc 1 172 0
		SLDDW		*-AR14[13], R43:R42
		SNOP		5
		SMOVIL		-152, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SSTDW		R43:R42, *AR10
	.loc 1 173 0
		SMOVIL		-152, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMOVIL		-160, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SSTDW		R43:R42, *AR10
	.loc 1 174 0
		SMOVIL		-152, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADD.LS		OR11,64,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-168, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SSTDW		R43:R42, *AR10
	.loc 1 176 0
		SMOVIL		0, R42
		SSTW		R42, *-AR14[15]
		SBR		.L4
		SNOP		6
.L5:
	.loc 1 178 0
		SLDW		*-AR14[15], R20
		SNOP		5
		SLDW		*-AR14[15], R42
		SNOP		5
		SSHFAR		31, R42, R42
		SMOV.M1		R42, R21
		SSHFLR		25, R20, R42
		SSHFLL		7, R21, R47
		SOR		R47, R42, R47
		SSHFLL		7, R20, R46
		SMOVIL		-168, R42
		SMOVIL		-1, R43
		SMOVIL		-56, R10
		SMOVIL		-1, R11
		SADDA.M1		R11:R10,AR14,OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOV.M1		R46, R44
		SMOV.M1		R47, R45
	;no-op trunc di R45:R44 to pdi R45:R44
		SMVAGA36.M1		R43:R42, AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, OR11
		SNOP		1
		SADDA.M1		R45:R44,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDDWM2.LS 		*AR1,VR5:VR4
		SNOP		7
		SMOVIL		-152, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTDW.LS 		VR5:VR4,*AR1
	.loc 1 180 0
		SMOVIL		-160, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDW.LS 		*AR1,VR3
		SNOP		7
		VSHFAR		15,VR3, VR3
		SMOVIL		-160, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 181 0
		SMOVIL		-168, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VLDW.LS 		*AR1,VR3
		SNOP		7
		VSHFAR		15,VR3, VR3
		SMOVIL		-168, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R43:R42
		SNOP		5
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 183 0
		SLDW		*-AR14[15], R22
		SNOP		5
		SLDW		*-AR14[15], R42
		SNOP		5
		SSHFAR		31, R42, R42
		SMOV.M1		R42, R23
		SSHFLR		26, R22, R42
		SSHFLL		6, R23, R49
		SOR		R49, R42, R49
		SSHFLL		6, R22, R48
		SMOV.M1		R48, R42
		SMOV.M1		R49, R43
	;no-op trunc di R43:R42 to pdi R43:R42
		SLDDW		*-AR14[12], R45:R44
		SNOP		5
		SMVAGA36.M1		R45:R44, OR11
		SNOP		1
		SADDA.M1		R43:R42,OR11,OR11
		SNOP		1
		SMVAAGL.M1		OR11, R43:R42
		SNOP		1
		SMVAAGH.M1		OR11, R43:R42
		SNOP		1
		SMOVIL		-160, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R45:R44
		SNOP		5
		SMVAGA36.M1		R45:R44, AR1
		SNOP		1
		VLDW.LS 		*AR1,VR3
		SNOP		7
		SMOVIL		-168, R50
		SMOVIL		-1, R51
		SADDA.M1		R51:R50,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R45:R44
		SNOP		5
		SMVAGA36.M1		R45:R44, AR1
		SNOP		1
		VLDW.LS 		*AR1,VR4
		SNOP		7
		VSBALE2		VR3,VR4,VR3
		SMVAGA36.M1		R43:R42, AR1
		SNOP		1
		VSTW.LS 		VR3,*AR1
	.loc 1 176 0
		SLDW		*-AR14[15], R42
		SNOP		5
		SADD.M1		1,R42,R42
		SSTW		R42, *-AR14[15]
.L4:
		SMOVIL		-84, R44
		SMOVIL		-1, R45
		SADDA.M1		R45:R44,AR14,AR10
		SNOP		1
		SLDW		*AR10, R42
		SNOP		5
		SADD.M1		1,R42,R42
		SSHFLR		31, R42, R43
		SADD.M1		R42,R43,R42
		SSHFAR		1, R42, R42
		SMOV.M1		R42, R43
		SLDW		*-AR14[15], R42
		SNOP		5
		SLT		R42, R43, R42
		SMOV.M1		R42, R0
	[R0]	SBR		.L5
		SNOP		6
	.loc 1 185 0
		SMOVIL		-152, R42
		SMOVIL		-1, R43
		SADDA.M1		R43:R42,AR14,AR10
		SNOP		1
		SLDDW		*AR10, R11:R10
		SNOP		5
		SBR		vfree
		SMOVIL		.L6, R62
		SMOVIH		.L6, R62
		SMOVIH4.L		.L6, R63
		SNOP		4
.L6:
	.loc 1 186 0
		SMOVIL		768, R6
		SMOVIL		0, R7
		SADDA.M1		R7:R6,AR7,AR7
		SNOP		2
		SLDDW		*+AR15[23], R31:R30
		SNOP		5
		SLDDW		*+AR15[24], R33:R32
		SNOP		5
		SLDDW		*+AR15[25], R35:R34
		SNOP		5
		SLDDW		*+AR15[26], R37:R36
		SNOP		5
		SLDDW		*+AR15[27], R39:R38
		SNOP		5
		SLDDW		*+AR15[28], R63:R62
		SNOP		5
		SADDA.M1		-16,AR14,AR15
		SNOP		1
		SLDDW		*++AR15[2], R7:R6
		SNOP		5
		SMVAGA36.M1		R7:R6, AR14
		SNOP		1
		SMVCGC.L		R63, BRReg
		SNOP		1
		SBR		R62
		SNOP		6
.LFE3:
	.size	DSP_fir_r8_h8_vc1, .-DSP_fir_r8_h8_vc1
	.section	.debug_frame,"",@progbits
.Lframe0:
	.4byte	.LECIE0-.LSCIE0
.LSCIE0:
	.4byte	0xffffffff
	.byte	0x1
	.string	""
	.byte	0x1
	.byte	0x7c
	.byte	0x4e
	.byte	0xc
	.byte	0xf
	.byte	0
	.align	3
.LECIE0:
.LSFDE0:
	.4byte	.LEFDE0-.LASFDE0
.LASFDE0:
	.4byte	.Lframe0
	.llong	.LFB3
	.llong	.LFE3-.LFB3
	.byte	0x4
	.4byte	.LCFI0-.LFB3
	.byte	0xe
	.byte	0x10
	.byte	0x8e
	.byte	0
	.byte	0x4
	.4byte	.LCFI1-.LCFI0
	.byte	0xc
	.byte	0xe
	.byte	0
	.byte	0x4
	.4byte	.LCFI2-.LCFI1
	.byte	0x5
	.byte	0x4e
	.byte	0x4
	.byte	0x5
	.byte	0x4f
	.byte	0x3
	.byte	0xb6
	.byte	0x6
	.byte	0xb7
	.byte	0x5
	.byte	0xb4
	.byte	0x8
	.byte	0xb5
	.byte	0x7
	.byte	0xb2
	.byte	0xa
	.byte	0xb3
	.byte	0x9
	.byte	0xb0
	.byte	0xc
	.byte	0xb1
	.byte	0xb
	.byte	0xae
	.byte	0xe
	.byte	0xaf
	.byte	0xd
	.align	3
.LEFDE0:
.text;
.Letext0:
	.section	.debug_info,"",@progbits
.Ldebug_info0:
	.4byte	0x5f2
	.2byte	0x2
	.4byte	.Ldebug_abbrev0
	.byte	0x8
	.byte	0x1
	.string	"GNU C 4.7.0"
	.byte	0x1
	.string	"../test.c"
	.string	"/cygdrive/c/Users/zhou hang/OneDrive/\346\241\214\351\235\242/DSP_fir_r8_h8/DSP_fir_r8_h8/Debug"
	.4byte	.Ldebug_ranges0+0
	.llong	0
	.llong	0
	.4byte	.Ldebug_line0
	.byte	0x2
	.byte	0x8
	.byte	0x7
	.string	"long long unsigned int"
	.byte	0x2
	.byte	0x1
	.byte	0x8
	.string	"unsigned char"
	.byte	0x2
	.byte	0x2
	.byte	0x7
	.string	"short unsigned int"
	.byte	0x2
	.byte	0x4
	.byte	0x7
	.string	"unsigned int"
	.byte	0x2
	.byte	0x4
	.byte	0x7
	.string	"long unsigned int"
	.byte	0x2
	.byte	0x1
	.byte	0x6
	.string	"signed char"
	.byte	0x2
	.byte	0x2
	.byte	0x5
	.string	"short int"
	.byte	0x2
	.byte	0x4
	.byte	0x5
	.string	"int"
	.byte	0x2
	.byte	0x8
	.byte	0x5
	.string	"long long int"
	.byte	0x2
	.byte	0x4
	.byte	0x5
	.string	"long int"
	.byte	0x2
	.byte	0x8
	.byte	0x7
	.string	"sizetype"
	.byte	0x2
	.byte	0x1
	.byte	0x6
	.string	"char"
	.byte	0x2
	.byte	0x4
	.byte	0x4
	.string	"float"
	.byte	0x2
	.byte	0x8
	.byte	0x4
	.string	"double"
	.byte	0x3
	.byte	0x1
	.string	"DSP_fir_r8_h8_vc1"
	.byte	0x1
	.byte	0xc
	.byte	0x1
	.llong	.LFB3
	.llong	.LFE3
	.4byte	.LLST0
	.byte	0x1
	.4byte	0x578
	.byte	0x4
	.string	"x"
	.byte	0x1
	.byte	0xd
	.4byte	0x585
	.byte	0x3
	.byte	0x91
	.byte	0xb0,0x7e
	.byte	0x4
	.string	"h"
	.byte	0x1
	.byte	0xe
	.4byte	0x58b
	.byte	0x3
	.byte	0x91
	.byte	0xa8,0x7e
	.byte	0x4
	.string	"r"
	.byte	0x1
	.byte	0xf
	.4byte	0x585
	.byte	0x3
	.byte	0x91
	.byte	0xa0,0x7e
	.byte	0x4
	.string	"nr"
	.byte	0x1
	.byte	0x10
	.4byte	0x10b
	.byte	0x3
	.byte	0x91
	.byte	0x9c,0x7e
	.byte	0x5
	.string	"MW_config_Addr"
	.byte	0x1
	.byte	0x13
	.4byte	0x591
	.byte	0x3
	.byte	0x91
	.byte	0xb0,0x7f
	.byte	0x5
	.string	"i"
	.byte	0x1
	.byte	0x2e
	.4byte	0x10b
	.byte	0x2
	.byte	0x91
	.byte	0x44
	.byte	0x5
	.string	"j"
	.byte	0x1
	.byte	0x2e
	.4byte	0x10b
	.byte	0x2
	.byte	0x91
	.byte	0x40
	.byte	0x5
	.string	"k"
	.byte	0x1
	.byte	0x2e
	.4byte	0x10b
	.byte	0x3
	.byte	0x91
	.byte	0xbc,0x7f
	.byte	0x5
	.string	"vr_len"
	.byte	0x1
	.byte	0x2f
	.4byte	0x10b
	.byte	0x3
	.byte	0x91
	.byte	0xac,0x7f
	.byte	0x5
	.string	"int_h_0"
	.byte	0x1
	.byte	0x31
	.4byte	0x597
	.byte	0x3
	.byte	0x91
	.byte	0xd4,0x7e
	.byte	0x5
	.string	"int_h_1"
	.byte	0x1
	.byte	0x31
	.4byte	0x597
	.byte	0x3
	.byte	0x91
	.byte	0xd0,0x7e
	.byte	0x5
	.string	"int_h_2"
	.byte	0x1
	.byte	0x31
	.4byte	0x597
	.byte	0x3
	.byte	0x91
	.byte	0xcc,0x7e
	.byte	0x5
	.string	"int_h_3"
	.byte	0x1
	.byte	0x31
	.4byte	0x597
	.byte	0x3
	.byte	0x91
	.byte	0xc8,0x7e
	.byte	0x5
	.string	"int_h_4"
	.byte	0x1
	.byte	0x31
	.4byte	0x597
	.byte	0x3
	.byte	0x91
	.byte	0xc4,0x7e
	.byte	0x5
	.string	"int_h_5"
	.byte	0x1
	.byte	0x31
	.4byte	0x597
	.byte	0x3
	.byte	0x91
	.byte	0xc0,0x7e
	.byte	0x5
	.string	"int_h_6"
	.byte	0x1
	.byte	0x31
	.4byte	0x597
	.byte	0x3
	.byte	0x91
	.byte	0xbc,0x7e
	.byte	0x5
	.string	"int_h_7"
	.byte	0x1
	.byte	0x31
	.4byte	0x597
	.byte	0x3
	.byte	0x91
	.byte	0xb8,0x7e
	.byte	0x5
	.string	"temp_vh_0"
	.byte	0x1
	.byte	0x32
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xe0,0x5
	.byte	0x5
	.string	"temp_vh_1"
	.byte	0x1
	.byte	0x32
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xc0,0x5
	.byte	0x5
	.string	"temp_vh_2"
	.byte	0x1
	.byte	0x32
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xa0,0x5
	.byte	0x5
	.string	"temp_vh_3"
	.byte	0x1
	.byte	0x32
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0x80,0x5
	.byte	0x5
	.string	"temp_vh_4"
	.byte	0x1
	.byte	0x32
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xe0,0x4
	.byte	0x5
	.string	"temp_vh_5"
	.byte	0x1
	.byte	0x32
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xc0,0x4
	.byte	0x5
	.string	"temp_vh_6"
	.byte	0x1
	.byte	0x32
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xa0,0x4
	.byte	0x5
	.string	"temp_vh_7"
	.byte	0x1
	.byte	0x32
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0x80,0x4
	.byte	0x5
	.string	"temp_vr_0_15"
	.byte	0x1
	.byte	0x34
	.4byte	0x5a9
	.byte	0x3
	.byte	0x77
	.byte	0x80,0x1
	.byte	0x6
	.string	"temp_vr_0_14"
	.byte	0x1
	.byte	0x34
	.4byte	0x5a9
	.byte	0x5
	.string	"temp_vx_0_15"
	.byte	0x1
	.byte	0x36
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xe0,0x3
	.byte	0x5
	.string	"temp_vx_1_16"
	.byte	0x1
	.byte	0x36
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xc0,0x3
	.byte	0x5
	.string	"temp_vx_2_17"
	.byte	0x1
	.byte	0x36
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xa0,0x3
	.byte	0x5
	.string	"temp_vx_3_18"
	.byte	0x1
	.byte	0x36
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0x80,0x3
	.byte	0x5
	.string	"temp_vx_4_19"
	.byte	0x1
	.byte	0x37
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xe0,0x2
	.byte	0x5
	.string	"temp_vx_5_20"
	.byte	0x1
	.byte	0x37
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xc0,0x2
	.byte	0x5
	.string	"temp_vx_6_21"
	.byte	0x1
	.byte	0x37
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0xa0,0x2
	.byte	0x5
	.string	"temp_vx_7_22"
	.byte	0x1
	.byte	0x37
	.4byte	0x59c
	.byte	0x3
	.byte	0x77
	.byte	0x80,0x2
	.byte	0x5
	.string	"temp_vx_0_31"
	.byte	0x1
	.byte	0x38
	.4byte	0x5b6
	.byte	0x3
	.byte	0x77
	.byte	0xc0,0x1
	.byte	0x5
	.string	"tmpr"
	.byte	0x1
	.byte	0x3a
	.4byte	0x5c3
	.byte	0x3
	.byte	0x91
	.byte	0xa0,0x7f
	.byte	0x5
	.string	"svb_temp_h"
	.byte	0x1
	.byte	0x3c
	.4byte	0x5c3
	.byte	0x3
	.byte	0x91
	.byte	0x80,0x7f
	.byte	0x5
	.string	"temp_x_offset_addr"
	.byte	0x1
	.byte	0x3e
	.4byte	0x585
	.byte	0x3
	.byte	0x91
	.byte	0xf8,0x7e
	.byte	0x6
	.string	"temp_r_offset_addr"
	.byte	0x1
	.byte	0x3f
	.4byte	0x5c3
	.byte	0x5
	.string	"temp_x_offset_addr_p_d"
	.byte	0x1
	.byte	0x40
	.4byte	0x5c9
	.byte	0x3
	.byte	0x91
	.byte	0xf0,0x7e
	.byte	0x5
	.string	"ttt"
	.byte	0x1
	.byte	0x46
	.4byte	0x5cf
	.byte	0x2
	.byte	0x77
	.byte	0
	.byte	0x5
	.string	"temp_in_r"
	.byte	0x1
	.byte	0x47
	.4byte	0x5dc
	.byte	0x3
	.byte	0x91
	.byte	0x98,0x7f
	.byte	0x5
	.string	"temp_in_r_high"
	.byte	0x1
	.byte	0x4a
	.4byte	0x5c3
	.byte	0x3
	.byte	0x91
	.byte	0x90,0x7f
	.byte	0x5
	.string	"temp_in_r_low"
	.byte	0x1
	.byte	0x4b
	.4byte	0x5c3
	.byte	0x3
	.byte	0x91
	.byte	0x88,0x7f
	.byte	0x5
	.string	"M2LD_temp1"
	.byte	0x1
	.byte	0xab
	.4byte	0x5c3
	.byte	0x3
	.byte	0x91
	.byte	0xe0,0x7e
	.byte	0x5
	.string	"M2LD_temp2"
	.byte	0x1
	.byte	0xab
	.4byte	0x5c3
	.byte	0x3
	.byte	0x91
	.byte	0xd8,0x7e
	.byte	0x5
	.string	"M2LD_Dst"
	.byte	0x1
	.byte	0xac
	.4byte	0x5ef
	.byte	0x3
	.byte	0x91
	.byte	0xe8,0x7e
	.byte	0x7
	.byte	0x1
	.string	"vfree"
	.byte	0x1
	.byte	0xb9
	.4byte	0x10b
	.byte	0x1
	.byte	0x8
	.byte	0
	.byte	0
	.byte	0x9
	.byte	0x1
	.4byte	0xfe
	.4byte	0x585
	.byte	0xa
	.byte	0xf
	.byte	0
	.byte	0xb
	.byte	0x8
	.4byte	0x578
	.byte	0xb
	.byte	0x8
	.4byte	0xfe
	.byte	0xb
	.byte	0x8
	.4byte	0x10b
	.byte	0xc
	.4byte	0x10b
	.byte	0x9
	.byte	0x1
	.4byte	0xfe
	.4byte	0x5a9
	.byte	0xa
	.byte	0xf
	.byte	0
	.byte	0x9
	.byte	0x1
	.4byte	0x10b
	.4byte	0x5b6
	.byte	0xa
	.byte	0xf
	.byte	0
	.byte	0x9
	.byte	0x1
	.4byte	0xfe
	.4byte	0x5c3
	.byte	0xa
	.byte	0x1f
	.byte	0
	.byte	0xb
	.byte	0x8
	.4byte	0x5a9
	.byte	0xb
	.byte	0x8
	.4byte	0x5b6
	.byte	0x9
	.byte	0x1
	.4byte	0x10b
	.4byte	0x5dc
	.byte	0xa
	.byte	0x1f
	.byte	0
	.byte	0xb
	.byte	0x8
	.4byte	0x5cf
	.byte	0x9
	.byte	0x1
	.4byte	0x14c
	.4byte	0x5ef
	.byte	0xa
	.byte	0xf
	.byte	0
	.byte	0xb
	.byte	0x8
	.4byte	0x5e2
	.byte	0
	.section	.debug_abbrev,"",@progbits
.Ldebug_abbrev0:
	.byte	0x1
	.byte	0x11
	.byte	0x1
	.byte	0x25
	.byte	0x8
	.byte	0x13
	.byte	0xb
	.byte	0x3
	.byte	0x8
	.byte	0x1b
	.byte	0x8
	.byte	0x55
	.byte	0x6
	.byte	0x11
	.byte	0x1
	.byte	0x52
	.byte	0x1
	.byte	0x10
	.byte	0x6
	.byte	0
	.byte	0
	.byte	0x2
	.byte	0x24
	.byte	0
	.byte	0xb
	.byte	0xb
	.byte	0x3e
	.byte	0xb
	.byte	0x3
	.byte	0x8
	.byte	0
	.byte	0
	.byte	0x3
	.byte	0x2e
	.byte	0x1
	.byte	0x3f
	.byte	0xc
	.byte	0x3
	.byte	0x8
	.byte	0x3a
	.byte	0xb
	.byte	0x3b
	.byte	0xb
	.byte	0x27
	.byte	0xc
	.byte	0x11
	.byte	0x1
	.byte	0x12
	.byte	0x1
	.byte	0x40
	.byte	0x6
	.byte	0x96,0x42
	.byte	0xc
	.byte	0x1
	.byte	0x13
	.byte	0
	.byte	0
	.byte	0x4
	.byte	0x5
	.byte	0
	.byte	0x3
	.byte	0x8
	.byte	0x3a
	.byte	0xb
	.byte	0x3b
	.byte	0xb
	.byte	0x49
	.byte	0x13
	.byte	0x2
	.byte	0xa
	.byte	0
	.byte	0
	.byte	0x5
	.byte	0x34
	.byte	0
	.byte	0x3
	.byte	0x8
	.byte	0x3a
	.byte	0xb
	.byte	0x3b
	.byte	0xb
	.byte	0x49
	.byte	0x13
	.byte	0x2
	.byte	0xa
	.byte	0
	.byte	0
	.byte	0x6
	.byte	0x34
	.byte	0
	.byte	0x3
	.byte	0x8
	.byte	0x3a
	.byte	0xb
	.byte	0x3b
	.byte	0xb
	.byte	0x49
	.byte	0x13
	.byte	0
	.byte	0
	.byte	0x7
	.byte	0x2e
	.byte	0x1
	.byte	0x3f
	.byte	0xc
	.byte	0x3
	.byte	0x8
	.byte	0x3a
	.byte	0xb
	.byte	0x3b
	.byte	0xb
	.byte	0x49
	.byte	0x13
	.byte	0x3c
	.byte	0xc
	.byte	0
	.byte	0
	.byte	0x8
	.byte	0x18
	.byte	0
	.byte	0
	.byte	0
	.byte	0x9
	.byte	0x1
	.byte	0x1
	.byte	0x87,0x42
	.byte	0xc
	.byte	0x49
	.byte	0x13
	.byte	0x1
	.byte	0x13
	.byte	0
	.byte	0
	.byte	0xa
	.byte	0x21
	.byte	0
	.byte	0x2f
	.byte	0xb
	.byte	0
	.byte	0
	.byte	0xb
	.byte	0xf
	.byte	0
	.byte	0xb
	.byte	0xb
	.byte	0x49
	.byte	0x13
	.byte	0
	.byte	0
	.byte	0xc
	.byte	0x35
	.byte	0
	.byte	0x49
	.byte	0x13
	.byte	0
	.byte	0
	.byte	0
	.section	.debug_loc,"",@progbits
.Ldebug_loc0:
.LLST0:
	.llong	.LFB3
	.llong	.LCFI0
	.2byte	0x2
	.byte	0x7f
	.byte	0
	.llong	.LCFI0
	.llong	.LCFI1
	.2byte	0x2
	.byte	0x7f
	.byte	0x10
	.llong	.LCFI1
	.llong	.LFE3
	.2byte	0x2
	.byte	0x7e
	.byte	0
	.llong	0
	.llong	0
	.section	.debug_aranges,"",@progbits
	.4byte	0x2c
	.2byte	0x2
	.4byte	.Ldebug_info0
	.byte	0x8
	.byte	0
	.2byte	0
	.2byte	0
	.llong	.LFB3
	.llong	.LFE3-.LFB3
	.llong	0
	.llong	0
	.section	.debug_ranges,"",@progbits
.Ldebug_ranges0:
	.llong	.LFB3
	.llong	.LFE3
	.llong	0
	.llong	0
	.section	.debug_line,"",@progbits
.Ldebug_line0:
