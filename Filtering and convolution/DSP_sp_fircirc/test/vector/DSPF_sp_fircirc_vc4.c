...
void DSPF_sp_fircirc_vc4(
    ...
    vector float temp_vx,temp_vr;
    float* ptr_x = (float*)x;
    vector float* x_addr;
    vector float temp_vr_index_1,temp_vr_index_0;
    /* 在栈开一个32 *4的空间 */
    ...
    // if(block_length <= 16 ){
    //     vector float * h_offset = (vector float *)(f_space + 16 - block_length); /* h广播后后面几位置0  */
        
    //     temp_vx = x[0];
    //     for( i  = 0; i < nh; i += 2){
    //     	x0_offset = (vector float *)(f_space+((index + i)&mod));
    //         x1_offset = (vector float *)(f_space+((index + i + 1)&mod));

    //         temp_vr = vec_svbcast((float)0);
           
    //         temp_vx_0_15_db[0] = vec_svbcast(h[i]); 
    //         temp_vh_0 = *h_offset;

    //         temp_vx_0_15_db[0] = vec_svbcast(h[i+1]); 
    //         temp_vh_1 = *h_offset;

    //         temp_h_x_0 = vec_muli(temp_vh_0,temp_vx);  /* 第一次特殊处理 */
    //         temp_vx_0_15_db[0] = temp_h_x_0;
    //         temp_vr = vec_add(temp_vr,*x0_offset);

    //         temp_h_x_1 = vec_muli(temp_vh_1,temp_vx);
    //         temp_vx_0_15_db[0] = temp_h_x_1;
    //         temp_vr = vec_add(temp_vr,*x1_offset);
    //         vec_st(temp_vr,0,r);
    //         // 
    //         // j = (block_length - ((index + i)&mod))&mod;
    //         // k = (block_length - ((index + i + 1)&mod))&mod;       /* 处理 当j || k  ==  block_length时，的情况 */

    //         j = block_length - ((index + i)&mod);
    //         k = block_length - (index + i + 1)&mod;
    //         for( ; k < nr; j += block_length,k += block_length){  /* 对于不同的h  循环次数相同 */
    //             temp_vr_index_0 = vec_ld(j,r);
    //             temp_vr_index_0 = vec_add(temp_vr_index_0,temp_h_x_0);
    //             vec_st(temp_vr_index_0,j,r);

    //             temp_vr_index_1 = vec_ld(k,r);
    //             temp_vr_index_1 = vec_add(temp_vr_index_1,temp_h_x_1);
    //             vec_st(temp_vr_index_1,k,r);                
    //         }
    //     }
    // }